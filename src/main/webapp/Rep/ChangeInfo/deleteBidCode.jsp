...
<%
    ...
    if (!rs.next()) {
    	out.println("Bid does not exist <div><a href='removeBid.jsp'>Try again</a></div>");
    }
    ...
    // Insert the new user into the database
		...
		ps.executeUpdate();
		ResultSet rs2=st.executeQuery("select max(amount) m from bidOn where auction_id= '" + auction_id+"';");
		ResultSet rs3=st.executeQuery("select current_price from auctions where auction_id= '" + auction_id+"';");
		if(rs2.next() && rs3.next()){
			if(Integer.parseInt(rs2.getString("m")) < Integer.parseInt(rs3.getString("current_price"))){
				String query3="update auctions set current_price='"+rs2.getString("m")+"' where auction_id='"+auction_id+"';";
			}
		}
		con.close();
		...
		%>
		<br>
		<a href='removeBid.jsp'>Go back to Bid Removal Page</a><%
    }
%>